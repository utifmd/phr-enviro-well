...
<div class="flex-col p-6 space-y-3">
    ...
    <div id="legendStatus" class="flex">
        <span class="flex items-center text-sm font-medium text-gray-900 dark:text-white me-3"><span class="flex w-2.5 h-2.5 bg-yellow-300 rounded-full me-1.5 flex-shrink-0"></span>
            {{ \App\Utils\Enums\WorkOrderStatusEnum::STATUS_SENT->value }}
        </span>
        <span class="flex items-center text-sm font-medium text-gray-900 dark:text-white me-3"><span class="flex w-2.5 h-2.5 bg-red-500 rounded-full me-1.5 flex-shrink-0"></span>
            {{ \App\Utils\Enums\WorkOrderStatusEnum::STATUS_DENIED->value }}
        </span>
        <span class="flex items-center text-sm font-medium text-gray-900 dark:text-white me-3"><span class="flex w-2.5 h-2.5 bg-green-500 rounded-full me-1.5 flex-shrink-0"></span>
                    ...
                    <dl class="divide-y mt-6 divide-gray-100">
                            ...
                            <dd class="space-y-3 items-center mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                @foreach($post->workOrders as $i => $wo)
                                    ...
                                    @elseif($wo['status'] == \App\Utils\Enums\WorkOrderStatusEnum::STATUS_DENIED->value)
                                        <span class="flex w-2.5 h-2.5 bg-red-300 rounded-full me-1.5 flex-shrink-0"></span>
                                    @else
                                        <span class="flex w-2.5 h-2.5 bg-green-300 rounded-full me-1.5 flex-shrink-0"></span>
                                    @endif
                                    Load {{$i +1}}
                                    </span>
                                    @if($i == 1)
                        ...
                        <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                            ...
                            <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                <img class="w-[150] h-[150]" src="{{$post->uploadedUrls[0]->url ?? '#'}}" alt="evidence well">
                            </dd>
                        </div>
                    </dl>
                </div>
